...
@Component("goals")
public class GoalsLeaderboardStrategy implements LeaderboardStrategy {

    private final StringRedisTemplate stringRedisTemplate;

    public GoalsLeaderboardStrategy(StringRedisTemplate stringRedisTemplate) {
        this.stringRedisTemplate = stringRedisTemplate;
    }

    @Override
    public List<LeaderboardEntryDto> getTopPlayers(int limit) {
        String key = "leaderboard:goals";
        if (!stringRedisTemplate.hasKey(key)) {
            System.out.println("DEBUG: Redis key not found: " + key + " for category goals");
        ...
        System.out.println("DEBUG: Fetching goals from Redis key: " + key + ". Result size: " + (results != null ? results.size() : 0));

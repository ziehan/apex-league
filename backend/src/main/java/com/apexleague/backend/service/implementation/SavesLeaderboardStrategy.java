...
@Component("saves")
public class SavesLeaderboardStrategy implements LeaderboardStrategy {

    private final StringRedisTemplate stringRedisTemplate;

    public SavesLeaderboardStrategy(StringRedisTemplate stringRedisTemplate) {
        this.stringRedisTemplate = stringRedisTemplate;
    }

    @Override
    public List<LeaderboardEntryDto> getTopPlayers(int limit) {
        String key = "leaderboard:saves";
        if (!stringRedisTemplate.hasKey(key)) {
            System.out.println("DEBUG: Redis key not found: " + key + " for category saves");
        ...
        System.out.println("DEBUG: Fetching saves from Redis key: " + key + ". Result size: " + (results != null ? results.size() : 0));

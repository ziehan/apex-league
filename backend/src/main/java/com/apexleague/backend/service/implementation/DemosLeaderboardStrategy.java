...
@Component("demos")
public class DemosLeaderboardStrategy implements LeaderboardStrategy {

    private final StringRedisTemplate stringRedisTemplate;

    public DemosLeaderboardStrategy(StringRedisTemplate stringRedisTemplate) {
        this.stringRedisTemplate = stringRedisTemplate;
    }

    @Override
    public List<LeaderboardEntryDto> getTopPlayers(int limit) {
        String key = "leaderboard:demos";
        if (!stringRedisTemplate.hasKey(key)) {
            System.out.println("DEBUG: Redis key not found: " + key + " for category demos");
        ...
        System.out.println("DEBUG: Fetching demos from Redis key: " + key + ". Result size: " + (results != null ? results.size() : 0));

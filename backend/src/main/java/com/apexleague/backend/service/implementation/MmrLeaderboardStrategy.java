...
@Component("mmr")
public class MmrLeaderboardStrategy implements LeaderboardStrategy {

    private final StringRedisTemplate stringRedisTemplate;

    public MmrLeaderboardStrategy(StringRedisTemplate stringRedisTemplate) {
        this.stringRedisTemplate = stringRedisTemplate;
    }

    @Override
    public List<LeaderboardEntryDto> getTopPlayers(int limit) {
        String key = "leaderboard:mmr";
        if (!stringRedisTemplate.hasKey(key)) {
            System.out.println("DEBUG: Redis key not found: " + key + " for category mmr");
        ...
        System.out.println("DEBUG: Fetching mmr from Redis key: " + key + ". Result size: " + (results != null ? results.size() : 0));
